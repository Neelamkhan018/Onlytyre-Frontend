...
const sendOtp = async () => {
  ...
  try {
    await axios.post(`${url.nodeapipath}/loginpage`, { emailOrMobile });
    setIsOtpSent(true);
  } catch (err) {
    console.error("Error sending OTP:", err);
    setError("Failed to send OTP. Try again.");
...
const verifyOtp = async () => {
  try {
    const res = await axios.post(`${url.nodeapipath}/verify-otp-login`, {
      emailOrMobile,
      otp,
    });

    if (res.data.message === "OTP verified") {
      navigate("/front-register", { state: { emailOrMobile } });
    }
  } catch (err) {
...
//     try {
//       // Send request to backend to generate and send OTP
//       const response = await axios.post("http://localhost:8000/loginpage", { emailOrMobile });
